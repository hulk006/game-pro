...
int main(int argc, char *argv[])
{
    ...
    if (int a = bind(sockfd, (struct sockaddr*)(&server_addr), sizeof(struct sockaddr)) < 0)
    {
    ...
    while (1)
    {
        int len = recv(newsockfd, &ai1, sizeof(buf), 0);

       // int l = read(newsockfd,&ai1,sizeof(ai1));
        std::cout<<"buf = "<< buf << std::endl;
        if (strcmp(buf, "exit\n") == 0)
            break;
        //fputs(buf, stdout);//向文件中写入
        strcpy( sendbuf, "hello");
        send(newsockfd, sendbuf, len, 0);
